      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <section className="rounded-xl border bg-white p-5 shadow-sm lg:p-7">
          ...
          <div className="mt-5 grid gap-5 md:grid-cols-2">
            ...
            <label className="space-y-2 text-sm font-medium">Company<ERPSelect value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="h-10 w-full rounded-md border bg-white px-3 font-normal"><ERPSelectOption>PT ZENIT TECHNOLOGY SOLUTION</ERPSelectOption></ERPSelect></label>
            <label className="space-y-2 text-sm font-medium">POS Profile<ERPSelect value={form.pos_profile} onChange={(e) => setForm({ ...form, pos_profile: e.target.value })} className="h-10 w-full rounded-md border bg-white px-3 font-normal"><ERPSelectOption>Usaha Jualan Lilin</ERPSelectOption><ERPSelectOption>Default POS Profile</ERPSelectOption></ERPSelect></label>
            <label className="space-y-2 text-sm font-medium md:col-span-2">Cashier<ERPSelect value={form.user} onChange={(e) => setForm({ ...form, user: e.target.value })} className="h-10 w-full rounded-md border bg-white px-3 font-normal"><ERPSelectOption>Administrator</ERPSelectOption><ERPSelectOption>Kasir 1</ERPSelectOption></ERPSelect></label>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="w-16 px-5 py-3">No.</th><th className="px-5 py-3">Mode of Payment</th><th className="px-5 py-3">Opening Amount</th><th className="w-16 px-5 py-3" /></tr></thead>
              <tbody className="divide-y">{balances.map((row, index) => <tr key={`${row.mode_of_payment}-${index}`}><td className="px-5 py-3 text-slate-500">{index + 1}</td><td className="px-5 py-3"><ERPSelect value={row.mode_of_payment} onChange={(e) => setBalances(balances.map((item, i) => i === index ? { ...item, mode_of_payment: e.target.value } : item))} className="h-10 w-full min-w-44 rounded-md border bg-white px-3"><ERPSelectOption value="">Pilih metode</ERPSelectOption><ERPSelectOption>Cash</ERPSelectOption><ERPSelectOption>QRIS</ERPSelectOption><ERPSelectOption>Bank Transfer</ERPSelectOption><ERPSelectOption>Credit Card</ERPSelectOption></ERPSelect></td><td className="px-5 py-3"><div className="flex h-10 min-w-48 items-center rounded-md border px-3"><span className="mr-2 text-slate-400">Rp</span><input type="number" min="0" value={row.opening_amount} onChange={(e) => setBalances(balances.map((item, i) => i === index ? { ...item, opening_amount: Number(e.target.value) } : item))} className="w-full outline-none" /></div></td><td className="px-5 py-3"><Button variant="ghost" size="icon" disabled={balances.length === 1} onClick={() => setBalances(balances.filter((_, i) => i !== index))}><Trash2 className="text-slate-400" /></Button></td></tr>)}</tbody>
