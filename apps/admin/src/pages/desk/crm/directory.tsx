...
function AccountForm({ open, account, accounts, onClose, onSaved }: { open: boolean; account: CRMAccount | null; accounts: CRMAccount[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{account ? "Edit perusahaan" : "Tambah perusahaan"}</DialogTitle><DialogDescription>Data account, hierarki induk, dan dimensi segmentasi.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-5"><div className="grid gap-4 sm:grid-cols-2">
    ...
    <Field label="Perusahaan induk"><ERPSelect value={form.parent_account_id} onChange={(e) => set("parent_account_id", e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="">Tidak ada (root)</ERPSelectOption>{accounts.filter((item) => item.id !== account?.id).map((item) => <ERPSelectOption key={item.id} value={item.id}>{item.name}</ERPSelectOption>)}</ERPSelect></Field>
    ...
    <Field label="Ukuran perusahaan"><ERPSelect value={form.company_size} onChange={(e) => set("company_size", e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="">Belum ditentukan</ERPSelectOption>{sizes.map((size) => <ERPSelectOption key={size} value={size}>{size}</ERPSelectOption>)}</ERPSelect></Field>
    ...
    <Field label="Status"><ERPSelect value={form.status} onChange={(e) => set("status", e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm">{accountStatuses.map((status) => <ERPSelectOption key={status} value={status}>{status}</ERPSelectOption>)}</ERPSelect></Field>
...
function ContactForm({ open, contact, accounts, presetAccountID, onClose, onSaved }: { open: boolean; contact: CRMContact | null; accounts: CRMAccount[]; presetAccountID?: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>{contact ? "Edit kontak" : "Tambah kontak"}</DialogTitle><DialogDescription>Profil individu dan perusahaan tempat kontak bekerja.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-5"><div className="grid gap-4 sm:grid-cols-2">
    ...
    <Field label="Perusahaan"><ERPSelect value={form.account_id} onChange={(e) => set("account_id", e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="">Independen</ERPSelectOption>{accounts.map((item) => <ERPSelectOption key={item.id} value={item.id}>{item.name}</ERPSelectOption>)}</ERPSelect></Field><Field label="Jabatan"><Input value={form.position} onChange={(e) => set("position", e.target.value)} /></Field>
...
function InteractionDialog({ selection, onClose }: { selection: Selection; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(value) => !value && onClose()}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>Riwayat interaksi — {name}</DialogTitle><DialogDescription>Panggilan, email, meeting, dan task tersimpan sebagai timeline.</DialogDescription></DialogHeader>
    <form onSubmit={submit} className="rounded-xl border bg-slate-50 p-4"><div className="grid gap-3 sm:grid-cols-2"><Field label="Tipe"><ERPSelect value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as CRMInteraction["type"] })} className="h-9 rounded-md border bg-white px-3 text-sm">{interactionTypes.map((type) => <ERPSelectOption key={type} value={type}>{type}</ERPSelectOption>)}</ERPSelect></Field><Field label="Status"><ERPSelect value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as CRMInteraction["status"] })} className="h-9 rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="completed">completed</ERPSelectOption><ERPSelectOption value="pending">pending</ERPSelectOption><ERPSelectOption value="cancelled">cancelled</ERPSelectOption></ERPSelect></Field><Field label="Subjek" wide><Input required value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="Follow up proposal" /></Field><Field label="Jatuh tempo"><Input type="date" value={form.due_date} onChange={(e) => setForm({ ...form, due_date: e.target.value })} /></Field><div className="sm:col-span-2"><Field label="Catatan"><Textarea rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></Field></div></div><div className="mt-3 flex justify-end"><Button type="submit" size="sm" disabled={saving}><MessageSquarePlus />{saving ? "Menyimpan..." : "Catat interaksi"}</Button></div></form>
...
export default function CRMDirectoryPage() {
  ...
  return <div className="min-h-screen bg-slate-50"><CRMPageHeader title="Kontak & Perusahaan" description="Kelola individu, account perusahaan, hierarki organisasi, segmentasi, dan seluruh riwayat interaksi." actions={<Button onClick={() => tab === "accounts" ? (setEditingAccount(null), setAccountOpen(true)) : (setEditingContact(null), setContactOpen(true))}><Plus />Tambah {tab === "accounts" ? "perusahaan" : "kontak"}</Button>} />
    <main className="space-y-5 p-6 lg:p-8"><Tabs value={tab} onValueChange={(value) => setTab(value as DirectoryTab)}><TabsList className="inline-flex h-auto rounded-xl border bg-white p-1"><TabsTrigger value="accounts" className="gap-2 rounded-lg px-4 py-2 text-sm font-semibold"><Building2 className="size-4" />Perusahaan</TabsTrigger><TabsTrigger value="contacts" className="gap-2 rounded-lg px-4 py-2 text-sm font-semibold"><UsersRound className="size-4" />Kontak</TabsTrigger></TabsList></Tabs>
      <section className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="flex flex-wrap gap-3 border-b p-4"><label className="relative min-w-64 flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input className="pl-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={tab === "accounts" ? "Cari perusahaan, industri, wilayah..." : "Cari nama, email, telepon, jabatan..."} /></label>{tab === "accounts" && <><ERPSelect value={industry} onChange={(e) => setIndustry(e.target.value)} className="h-9 rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="">Semua industri</ERPSelectOption>{industries.map((value) => <ERPSelectOption key={value}>{value}</ERPSelectOption>)}</ERPSelect><ERPSelect value={size} onChange={(e) => setSize(e.target.value)} className="h-9 rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="">Semua ukuran</ERPSelectOption>{sizes.map((value) => <ERPSelectOption key={value}>{value}</ERPSelectOption>)}</ERPSelect><ERPSelect value={region} onChange={(e) => setRegion(e.target.value)} className="h-9 rounded-md border bg-white px-3 text-sm"><ERPSelectOption value="">Semua wilayah</ERPSelectOption>{regions.map((value) => <ERPSelectOption key={value}>{value}</ERPSelectOption>)}</ERPSelect></>}<div className="relative"><Tags className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input className="w-44 pl-9" value={tag} onChange={(e) => setTag(e.target.value)} placeholder="Filter tag" /></div></div>
