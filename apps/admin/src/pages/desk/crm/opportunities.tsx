...
function OpportunityForm({ open, opportunity, pipeline, initialStage, onClose, onSaved }: { open: boolean; opportunity: CRMOpportunity | null; pipeline: CRMPipeline; initialStage?: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{opportunity ? "Edit deal" : "Tambah deal"}</DialogTitle><DialogDescription>Nilai, probabilitas, owner, dan estimasi closing akan digunakan dalam forecast.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-5"><div className="grid gap-4 sm:grid-cols-2">
    ...
    <Field label="Perusahaan"><select value={form.account_id} onChange={(e) => setForm({ ...form, account_id: e.target.value, contact_id: "" })} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><option value="">Tanpa perusahaan</option>{pipeline.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field>
    <Field label="Kontak"><select value={form.contact_id} onChange={(e) => set("contact_id", e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><option value="">Tanpa kontak</option>{contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.first_name} {contact.last_name}</option>)}</select></Field>
    <Field label="Stage"><select required value={form.stage_id} onChange={(e) => changeStage(e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm">{pipeline.stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</select></Field>
    <Field label="Sales rep"><select value={form.owner_id} onChange={(e) => set("owner_id", e.target.value)} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><option value="">Unassigned</option>{pipeline.sales_reps.map((rep) => <option key={rep.id} value={rep.id}>{rep.name}</option>)}</select></Field>
...
function ForecastView({ pipeline }: { pipeline: CRMPipeline }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-end gap-3 rounded-2xl border bg-white p-4"><Field label="Dari"><Input type="date" value={from} onChange={(e) => setFrom(e.target.value)} /></Field><Field label="Sampai"><Input type="date" value={to} onChange={(e) => setTo(e.target.value)} /></Field><Field label="Sales rep"><select value={owner} onChange={(e) => setOwner(e.target.value)} className="h-9 min-w-48 rounded-md border bg-white px-3 text-sm"><option value="">Semua sales rep</option>{pipeline.sales_reps.map((rep) => <option key={rep.id} value={rep.id}>{rep.name}</option>)}</select></Field></div>
...
export default function CRMOpportunitiesPage() {
  ...
  return <div className="min-h-screen bg-slate-50"><CRMPageHeader title="Peluang & Deal" description="Kelola sales pipeline, probabilitas kemenangan, closing, dan revenue forecast." actions={<><Button variant="outline" onClick={() => setStageOpen(true)}><Settings2 />Atur stage</Button><Button onClick={() => { setEditing(null); setInitialStage(undefined); setFormOpen(true); }}><Plus />Tambah deal</Button></>} />
    <main className="space-y-5 p-5 lg:p-8"><div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div className="inline-flex w-fit rounded-xl border bg-white p-1"><button onClick={() => setView("pipeline")} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold ${view === "pipeline" ? "bg-blue-600 text-white" : "text-slate-600"}`}><Columns3 className="size-4" />Pipeline</button><button onClick={() => setView("forecast")} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold ${view === "forecast" ? "bg-blue-600 text-white" : "text-slate-600"}`}><BarChart3 className="size-4" />Forecast</button></div>{view === "pipeline" && <div className="flex flex-wrap gap-3"><label className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input className="w-72 pl-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari deal..." /></label><select value={owner} onChange={(e) => setOwner(e.target.value)} className="h-9 rounded-md border bg-white px-3 text-sm"><option value="">Semua sales rep</option>{pipeline.sales_reps.map((rep) => <option key={rep.id} value={rep.id}>{rep.name}</option>)}</select></div>}</div>
