      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        ...
        <form onSubmit={submit} className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Sumber lead"><select className="h-9 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={form.source || "website"} onChange={(e) => set("source", e.target.value)}>{sources.map((source) => <option key={source} value={source}>{source.replaceAll("_", " ")}</option>)}</select></Field>
            ...
            <Field label="Status"><select className="h-9 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={form.status || "new"} onChange={(e) => set("status", e.target.value)}>{leadStatuses.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}</select></Field>
